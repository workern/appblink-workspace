...
import { createOrderNotification } from '../../apps/nikat/orders';
    ...
    .runTransaction(async (t) => {
      ...
      if (user != null && transaction?.state == TRANSACTION_STATE_PENDING) {
        ...
        if (transaction.reason == TransactionReason.DEPOSIT) {
          ...
          const customerSpaceRef = userRef.collection('mySpaces').doc('nikat');
          ...
          let shop: ShopOwnerRecord;
          if (shopId) {
            const shopDoc = await t.get(
              db.doc(`apps/${APPID.NIKAT_SHOP_MANAGER}/shops/${shopId}`)
            );
            shop = shopDoc.data() as ShopOwnerRecord;
          }
          const globalNikatAppOrderRef = db.doc(`apps/${APPID.NIKAT}/orders/${orderId}`);
          ...
          if (shop && order) {
            const globalSangrahAppOrderRef = db.doc(
              `apps/${APPID.NIKAT_SHOP_MANAGER}/orders/${orderId}`
            );
            const newOrder = {
              ...order,
              status: OrderStatuses.Processing,
            ...
            t.create(
              db.doc(
                `apps/${APPID.NIKAT_SHOP_MANAGER}/workspaces/${shop.id}/orders/${orderId}`
              ),
              newOrder
            );
          }
    ...
    .runTransaction(async (t) => {
      ...
      if (user != null && transaction?.state == TRANSACTION_STATE_PENDING) {
        ...
        if (transaction.reason == TransactionReason.NIKAT_APP_ORDER_PAYMENT) {
          ...
          let sangrahShop: ShopOwnerRecord;
          if (shopId && orderId) {
            const sangrahShopDoc = await t.get(
              db.doc(`apps/${APPID.NIKAT_SHOP_MANAGER}/shops/${shopId}`)
            );
            sangrahShop = sangrahShopDoc.data() as ShopOwnerRecord;

            const globalNikatAppOrderRef = db.doc(
              `apps/${APPID.NIKAT}/orders/${orderId}`
            );
