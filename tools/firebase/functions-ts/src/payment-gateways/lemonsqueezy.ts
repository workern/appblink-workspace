    ...
    try {
      ...
      if (transactionId) {
        ...
        if (eventName === 'order_created') {
          ...
          const websiteId = notes?.website_id;
          if (websiteId && notes?.source === eGiftsAppSpaceId) {
            const buyerEmail =
            ...
            const buyerName =
              ...
              event.data?.attributes?.customer_name ||
              '';

            if (buyerEmail || buyerName) {
              const websiteRef = db.doc(
                `apps/eGiftsApp/customWebsites/${websiteId}`
              );
              const userWebsiteRef = db.doc(
                `users/${transaction.uid}/mySpaces/eGiftsApp/valentineWebsites/${websiteId}`
              );
